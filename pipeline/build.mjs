...
// ONE feed, TWO cfgs: route_type splits the bundle (3 and 11 = road transport,
// 0/1/2/6 = the fixed-track side), agency_id splits it further inside each.
//
// LINE KEYS. Seven operators here number their lines from 1: Metro line 1,
// Metrobús line 1, Trolebús line 1, Cablebús line 1, the Tren Ligero, the
// Suburbano and the Tren El Insurgente all exist, and "1" alone names seven
// different services. So every key carries its operator:
//   M1…M9, MA, MB, M12   Metro           B1…B7, BSL01   Metrobús
//   T1…T13               Trolebús        CB1…CB3        Cablebús
//   R…                   RTP             C…             corredores concesionados
//   P1…P13               Pumabús         TL / FS / TI   Tren Ligero, Suburbano,
//                                                        Tren El Insurgente
// RTP's four "Ordinario1 L1"…"Ordinario4 L1" routes are one line under four
...
const MODES = [{
  mode: 'bus', label: 'road transport', osmFile: 'data/osm/mexico.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['T1']),
  feeds: [
    // route_type 11 is there for a single route: SEMOVI's Trolebús Línea 13,
    // the elevated one, which the feed types correctly while the other eleven
    // trolleybus lines ride as plain 3 under agency TROLE.
    { tag: 'cdmx', dir: 'data/gtfs', mapKey: lineKey, routeTypes: ['3', '11'],
      trolley: (r) => r.agency_id === 'TROLE' || r.agency_id === 'SEMOVI' || r.route_type === '11',
      mline: (r) => r.agency_id === 'MB' },
  ],
}];
