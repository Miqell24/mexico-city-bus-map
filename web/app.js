// Frontend: MapLibre GL + OpenFreeMap vector tiles (positron) + SEMOVI line layers
...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: GTFS SEMOVI CDMX (via mobilitydatabase.org)' }));
  ...
  document.getElementById('count').textContent =
    `(${nBus - nTro - nMB} bus · ${nTro} trolleybus · ${nMB} Metrobús · ${nTram} rail & gondola)`;
  ...
  const PALETTE = [
    ...
    // the official line colours from the feed (uppercase — exactly as the pipeline
    // emits them from routes.txt). The styleimagemissing fallback below covers
    // anything missed, but a registered icon is one less canvas at first paint.
    ['#0071C1', '#003357'], // M2,
    ['#118749', '#083d21'], // M8,
    ['#118751', '#083d24'], // MB,
    ['#1F5AF0', '#0e286c'], // TL,
    ['#4EC3E0', '#235865'], // CB1, CB2, CB3,
    ['#512826', '#241211'], // M9,
    ['#7FBCAA', '#39554c'], // M4,
    ['#83332E', '#3b1715'], // TI,
    ['#A02D96', '#481444'], // MA,
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: SEMOVI Ciudad de México';
  ...
  map.jumpTo({ center: [-99.135, 19.400], zoom: 11.0 });
}
